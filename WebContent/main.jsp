...
<body style="background-image: url('./images/login_bg_0.jpg');">
	<div class="container-fluid" style="width: 100%;">
		...
		<a href="http://192.168.211.128:50070/explorer.html#/"><button class="btn btn-lg btn-warning btn-block" type="button">查看爬取结果</button></a> --%>
			<center>
				<button type="button" class="btn btn-danger" onclick="start();">一键爬取所有数据</button>
			</center>
			<center>
				<div class="btn-group">
					<button type="button" class="btn btn-primary">爬取线程个数</button>
					<button type="button" class="btn btn-primary dropdown-toggle"
						data-toggle="dropdown">
						<span class="caret"></span> <span class="sr-only">选择线程个数</span>
					</button>
					<ul class="dropdown-menu" role="menu">
						<li><a onclick="changeT('one');">5个线程爬取</a></li>
						<li class="divider"></li>
						<li><a onclick="changeT('two');">10个线程爬取</a></li>
					</ul>
				</div>
			</center>
			<center>
				<span id="show">5个线程爬取</span>
			</center>
			<table style="text-align: left" id="userPager"
				...
				<tbody>
					...
					<c:forEach items="${requestScope.allUrl}" var="data" varStatus="c">
						<tr class="bg-danger">
							<td>${c.count}</td>
							<td>${data.url}</td>
							<td>${data.address}</td>
			...
			<div align="center">
				<div class="pagination">
					<ul>
						<li><a
							href="${pageContext.request.contextPath}/allUrl?page=${requestScope.page-1}">上一页</a></li>
						<li><a
							href="${pageContext.request.contextPath}/allUrl?page=${requestScope.page+1}">下一页</a></li>
					</ul>
				</div>
			</div>
		</div>
